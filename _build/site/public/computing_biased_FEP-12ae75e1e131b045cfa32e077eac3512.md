...
techniques, often rooted in Zwanzig's free energy perturbation (Robert
W. Zwanzig 1954b), enable the recovery of unbiased distributions.
...
biased ensembles; for a comprehensive overview, see Ref. (Hénin et al.
2022).
...
to a perturbed one $H_1$ via Zwanzig's formula (Robert W. Zwanzig
1954a),
...
(Mitchell and McCammon 1991) at finite switching rates $ds/dt$, a clear
sign of non-equilibrium. Multiconfiguration TI (MCTI) (Straatsma and
McCammon 1991) addresses this issue by calculating proper ensemble
...
of Jarzynski's identity and steered molecular dynamics (Hummer 2001;
Park and Schulten 2004) --- see Section
...
restraint(Straatsma and McCammon 1991):
...
(Torrie and Valleau 1977; Kästner 2011), is one of the earliest
...
concepts (Roux 1995). The Weighted Histogram Analysis Method (WHAM)
...
histograms into a single distribution (see also (Souaille and Roux 2001;
Hub, De Groot, and Van Der Spoel 2010)). The Multistate Bennett
Acceptance Ratio (MBAR) method provides an equivalent, statistically
optimal alternative to WHAM (Shirts and Chodera 2008; Tan et al. 2012).

An alternative that avoids explicit offset estimation is umbrella
integration (UI) (Kästner and Thiel 2005), which reconstructs the mean
...
(Marinova and Salvalaglio 2019; Bjola and Salvalaglio 2024; Serse et al.
2024).
...
refines the bias toward uniform sampling (Mezei 1987), while local
elevation US introduces a history-dependent bias reminiscent of early
metadynamics (Huber, Torda, and Van Gunsteren 1994; Hansen and
Hünenberger 2010; Laio and Parrinello 2002). Multidimensional
formulations (Bartels and Karplus 1997; Kästner 2009) treat coupled
coordinates, and hybrid schemes---such as self-learning adaptive
variants (Wojtas-Niziurski et al. 2013) or replica-exchange US (Jiang et
al. 2012)---enable efficient, massively parallel implementations. These
developments confirm US remains a versatile and widely used framework
...
$\partial\xi/\partial \mathbf{r}$ (Goldstein, Poole, and Safko 2014; Van
Kampen and Lodder 1984) -- are used as an alternative to harmonic
restraints to enforce a specific value of the CV $\xi(\mathbf {r})=s$
using algorithms like SHAKE (Ryckaert, Ciccotti, and Berendsen 1977). In
this case, one needs to apply an unbiasing factor
$Z_{\xi}= \sum_i \left|\partial {\xi}/\partial \mathbf r_i\right|^2 / m_i$
to correct for momentum loss along the constraint(Carter et al. 1989).
...
a curvature correction(Sprik and Ciccotti 1998). The case of
multidimensional reaction constants is discussed in Ref.(Ciccotti 1991).
