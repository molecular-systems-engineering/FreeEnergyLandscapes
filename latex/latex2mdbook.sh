...
pandoc $1 --from=latex --to=markdown+tex_math_double_backslash  --citeproc  --reference-location=block --bibliography=`dirname $1`/../additional.bib  --bibliography=`dirname $1`/../zotero.bib --metadata reference-section-title="References" --lua-filter=`dirname $0`/textbox_div.lua --lua-filter=`dirname $0`/math_fix.lua  | `dirname $0`/convert_dollars.sh  | `dirname $0`/postprocess_refs.py |  pandoc --from=markdown --to=markdown   > $out
