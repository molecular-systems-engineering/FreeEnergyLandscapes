...
sed 's/\\,/ /g' $1 | pandoc  --from=latex  --to=markdown+tex_math_double_backslash  --citeproc  --reference-location=block --bibliography=`dirname $1`/../additional.bib  --bibliography=`dirname $1`/../zotero.bib --metadata reference-section-title="References" --lua-filter=`dirname $0`/myst_figures.lua  > $out
